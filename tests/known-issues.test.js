...
// Split into two top-level groups so `node --test` output makes it obvious at
// a glance which SIDE issues are fixed (asserting correct behavior — any
// failure here is a real regression) versus still open (asserting today's
// known-buggy behavior as a tripwire — a failure there means the bug got
// fixed and the test should be moved up to "Fixed issues" with a flipped
// assertion, not treated as a regression).

describe('Fixed issues (regressions guarded — these assert CORRECT behavior)', () => {
...
describe('Known open issues (expected to fail until fixed — assert TODAY\'S BUGGY behavior)', () => {
  describe('SIDE-27: combined multi-character edits within the same length', () => {
    test('[OPEN] two simultaneous substitutions are not detected as characterSubstitution', () => {
  ...
  describe('SIDE-28: binary fraud risk level', () => {
    test('[OPEN] every suspicious match is labeled "fraud" regardless of confidence', () => {
